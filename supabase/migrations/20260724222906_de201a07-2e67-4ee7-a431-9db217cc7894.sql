...
BEGIN
  ...
  -- Qualify every column with the table alias so none can be resolved to the
  -- OUT parameter names (call_event_id / started_at / is_creator) declared in
  -- RETURNS TABLE. Without this, ON CONFLICT (call_event_id, user_id) is
  -- ambiguous — Postgres 42702.
