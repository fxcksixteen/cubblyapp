...
describe("legacy group-ring bridge (compat: 0.4.26 caller -> 0.4.27 client)", () => {
  it("forwards a ring received on the legacy topic to the group handler", async () => {
    const { setLegacyGroupRingHandler, deliverLegacyGroupRing } = await import("./legacyGroupRingBridge");
    const seen: any[] = [];
    const unregister = setLegacyGroupRingHandler((p) => seen.push(p));
    deliverLegacyGroupRing({ targetId: "user-1", callEventId: "evt-1" });
    expect(seen).toHaveLength(1);
    unregister();
    // After unregister (provider unmounted) nothing is delivered.
    deliverLegacyGroupRing({ targetId: "user-1", callEventId: "evt-2" });
    expect(seen).toHaveLength(1);
  });

  it("is a no-op when no handler is registered", async () => {
    const { deliverLegacyGroupRing } = await import("./legacyGroupRingBridge");
    expect(() => deliverLegacyGroupRing({ targetId: "user-1" })).not.toThrow();
  });

  it("de-dupes a ring that arrives on BOTH topics (0.4.27 caller dual-sends)", async () => {
    const { setLegacyGroupRingHandler, deliverLegacyGroupRing } = await import("./legacyGroupRingBridge");
    // Mirror of GroupCallContext's handleGroupRing dedupe.
    const handled = new Map<string, number>();
    const rings: string[] = [];
    const handler = (payload: any) => {
      const key = String(payload.callEventId);
      if (handled.has(key) && Date.now() - handled.get(key)! < 60_000) return;
      handled.set(key, Date.now());
      rings.push(key);
    };
    const unregister = setLegacyGroupRingHandler(handler);
    handler({ callEventId: "evt-9" });          // arrived on group-global
    deliverLegacyGroupRing({ callEventId: "evt-9" }); // same ring via legacy topic
    expect(rings).toEqual(["evt-9"]);           // rings exactly once
    unregister();
  });
});
