...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    newFeatures: [
      ...
      "Profiles now have Mutual Friends and Mutual Servers tabs.",
      "Shop, Private Notes and Honey now show their icon in the top bar, like Friends.",
      "Cubbly now lives in your system tray. Closing the window keeps it running in the background — right-click the tray icon and choose Quit Cubbly to fully exit.",
      "Unread messages, mentions and friend requests now show as a red badge on the Cubbly taskbar icon.",
      "When someone has a custom status while playing something, their status and activity now take turns in the DM sidebar and friends list.",
      "Text is no longer selectable across the whole desktop app — only chat messages, notes, bios and statuses.",
      "New Billing tab icon, plus a refreshed Update Logs icon.",
    ],
    bugFixes: [
      "Screen shares now start at, and stay at, the quality you picked instead of easing up to it.",
      "Honey subscription renewal dates now roll forward correctly instead of showing a past date.",
      "Monthly gems in Billing no longer show as \"--\" while loading.",
      "Older note attachments open as images again instead of downloading as unreadable files.",
      "Honey subscribers no longer see the \"upgrade to use these\" note in note sharing options.",
      "Annual Honey now correctly reads 6,000 gems up front.",
      "Activity icons no longer sit inside a square plate.",
      "Icons load instantly everywhere instead of popping in.",
      "The app data loading timeouts some of you hit are fixed.",
      "Emoji suggestions show up again while typing.",
      "iOS push notifications no longer stop working because of one bad device.",
      "The error screen now shows Cubbly instead of a stray emoji.",
    ],
  },  {
  ...
  },
  {
    ...
    hero: bearImage,
    newFeatures: [
      "Screen shares are dramatically sharper. Cubbly was getting stuck on a low-quality fallback almost every time you shared — even on a powerful graphics card — and staying there for the rest of the share. It now gives the encoder time to warm up, and switches back the moment your GPU is ready.",
      "Cubbly now tunes each share for what you're actually sharing. Games and video stay smooth; documents, code and browsers stay sharp and readable. It detects which one you're sharing automatically, and you can override it any time with the new Motion / Text buttons in the share picker — your choice is remembered per window.",
      "Starting a screen share no longer spikes your ping. It now eases into full quality instead of grabbing all your upload at once, so your voice call stays clear while the share ramps up.",
      "Cubbly takes up a lot less room. The download dropped from 117 MB to 106 MB, the installed app went from 372 MB to 311 MB, and leftover update downloads — which could quietly pile up to hundreds of megabytes — are now cleaned up automatically.",
      "Group calls show each person's ping on their tile, so you can see whose connection is struggling instead of guessing.",
    ],
    bugFixes: [
      ...
      "Calls hold up better on rough networks. Cubbly was giving up on its backup connection route too early and leaving calls with nothing to fall back on when the direct route degraded.",
      "Screen share audio is noticeably less delayed.",
      "Group call rings are much more reliable and no longer get dropped on the way to you.",
      "The \"stream paused\" message is finally accurate. It used to appear while someone was sharing perfectly normally — any time the shared window simply wasn't moving. It now only shows when video really has stopped, appears everywhere the stream is shown instead of only in fullscreen, and sits over a blurred still of the share instead of a black box.",
      "Rich game details — your map, score, agent, Roblox experience — now actually save and show to your friends. They were being silently thrown away.",
  ...
  },
  {
    ...
    newFeatures: [],
    bugFixes: [
      "Fixed screensharing freezing the app on every share.",
      "Fixed the screenshare native module not being rebuilt, so the improvements from the last few patches actually take effect.",
    ],
  },
  {
    version: "0.4.24",
