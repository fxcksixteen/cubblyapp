...
interface ScreenSharePickerProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (type: ScreenShareType, options: { audio: boolean; fps: number; quality: string; sourceId?: string; sourceName?: string }) => void;
}
...
const ScreenSharePicker = ({ isOpen, onClose, onSelect }: ScreenSharePickerProps) => {
    ...
    try {
      ...
      if (type === "screen") {
        ...
        if (screens.length === 1) {
          ...
          onSelect(type, { audio: shareAudio, fps, quality, sourceId: screens[0].id, sourceName: screens[0].name });
          return;
        }
  ...
  const handleSourceSelect = (source: DesktopSource) => {
    onSelect(pickingType || "screen", { audio: shareAudio, fps, quality, sourceId: source.id, sourceName: source.name });
  };
