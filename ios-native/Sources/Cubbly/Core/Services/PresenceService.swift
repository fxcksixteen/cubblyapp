...
final class PresenceService: ObservableObject {
    ...
    /// Called when the app enters the background. Untracks presence and
    /// pauses the heartbeat so the user correctly appears offline to peers
    ...
    private func reconcileOnlineFromDatabase() async {
        ...
        do {
            ...
            // Trust the DB as the source of truth. Intersect with our
            // realtime set (to keep freshly-joined users the DB hasn't
            // seen yet) and union with the DB set. This removes stale
            // entries the realtime channel missed leaving.
            await MainActor.run {
                self.onlineUserIDs = dbIds
            }
        } catch {
