...
struct YouView: View {
    ...
    var body: some View {
        ...
        .onChange(of: avatarPick) { _, newValue in
            ...
            Task {
                if let url = await ProfilePhotoUploader.upload(item: item, kind: .avatar, userID: uid) {
                    session.setLocalAvatarURL(url)
                }
        ...
        .onChange(of: bannerPick) { _, newValue in
            ...
            Task {
                if let url = await ProfilePhotoUploader.upload(item: item, kind: .banner, userID: uid) {
                    session.setLocalBannerURL(url)
                }
                await session.reloadProfile()
