...
struct ChatView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // On iOS 26+ we draw our OWN topbar so there are no Liquid Glass
            // pills around the back arrow / call buttons. The native
            // `UINavigationController` still lives underneath (hidden) so the
            // interactive-pop swipe-back gesture remains exactly as-is.
        ...
        .applyPreIOS26Toolbar { chatToolbar }
        .nativeEdgeSwipeBack()
    ...
    // MARK: - iOS 26+ custom topbar (no Liquid Glass)

    /// Draws the chat topbar manually so iOS 26's automatic Liquid Glass pills
    /// don't wrap the back arrow and call buttons. The navigation controller
    /// is hidden (`.toolbar(.hidden, for: .navigationBar)`) but its interactive
    /// pop-gesture recognizer stays active via `NativeEdgeSwipeBackEnabler`.
    @available(iOS 26.0, *)
    ...
    // MARK: - Toolbar (pre-iOS 26 — native nav chrome preserved for swipe)
    ...
    private var composer: some View {
        ...
        return HStack(spacing: 10) {
            Button {
                ...
                // and bring the keyboard back.
                if attachPanelOpen {
                    // Remove panel first, then bring keyboard back once the
                    // panel's layout space is gone — prevents the two from
                    // fighting and causing a jump.
                    attachPanelOpen = false
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        composerFocused = true
                    }
                } else {
                    let kbWasOpen = composerFocused
                    composerFocused = false
                    if kbWasOpen {
                        // Defer panel until the keyboard starts collapsing so
                        // the two layout changes don't fight each other.
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                ...
                ZStack {
                    Circle()
                        .fill(attachPanelOpen ? Theme.Colors.primary : Theme.Colors.bgTertiary)
                        .frame(width: 36, height: 36)
                    Image(systemName: "plus")
                        .font(.system(size: 18, weight: .semibold))
                        .foregroundStyle(attachPanelOpen ? .white : Theme.Colors.textSecondary)
                        .rotationEffect(.degrees(attachPanelOpen ? 45 : 0))
                }
                .animation(.easeInOut(duration: 0.25), value: attachPanelOpen)
            }
            .buttonStyle(.plain)
...
private struct DiscordStyleBubble: View {
    ...
    var body: some View {
        ...
        // Make the ENTIRE row area (including empty space to the right of
        // short messages) respond to long-press and swipe gestures, not
        // just the text / avatar content.
        .contentShape(Rectangle())
        // Swipe-to-reply: leftward drag reveals the reply arrow and fires the
        // reply action when the threshold is crossed. Uses minimumDistance: 20
        // so the gesture doesn't steal vertical scroll touches (the old 0-distance
        // DragGesture was eating scrolling). `.simultaneousGesture` lets the
        // ScrollView's vertical pan still fire. Rightward drags (swipe-back)
        // are explicitly ignored so the native pop gesture stays untouched.
        .simultaneousGesture(
            DragGesture(minimumDistance: 20)
                .onChanged { value in
                    let h = value.translation.width
                    let v = value.translation.height
                    // Ignore mostly-vertical drags (scrolling) and rightward drags (swipe-back).
                ...
                .onEnded { value in
                    if -value.translation.width >= replyThreshold {
                        onSwipeReply()
                    }
        ...
        // Long-press for the action menu — now coexists with the swipe gesture
        // above. The press tint is delayed so a quick rightward swipe never
        // "grabs" the bubble visually.
...
/// NOTE: `.textSelection(.enabled)` was removed because its internal
/// long-press gesture recognizer blocked the parent bubble's
/// `.onLongPressGesture` from firing — making the message action menu
/// unreachable on text-heavy messages. Users can still copy text via the
/// "Copy Text" button in that menu.
private struct LinkifiedText: View {
    let content: String
    var body: some View {
        if let attributed = try? AttributedString(
            markdown: linkifyMarkdown(content),
...
// MARK: - Conditional toolbar helper

/// On iOS 26+ we hide the system back button and glass background so zero
/// Liquid Glass chrome is visible. `NativeEdgeSwipeBackEnabler` (which
/// re-asserts on every layout pass) keeps the interactive pop gesture alive
/// even though the back button is hidden. Our own `customTopBar` HStack
/// drawn inside the VStack provides the visual chrome instead.
///
/// On pre-iOS 26 we use the standard toolbar with a solid background.
private extension View {
    @ViewBuilder
    func applyPreIOS26Toolbar<T: ToolbarContent>(@ToolbarContentBuilder _ toolbar: () -> T) -> some View {
        if #available(iOS 26.0, *) {
            self
